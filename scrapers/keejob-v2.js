...
export async function scrapeKeejobV2() {
  const jobs = [];
  const baseUrl = 'https://www.keejob.com';
  
  try {
    console.log('🔍 Scraping Keejob (v2)...');
    
    // Essayer plusieurs pages
    for (let page = 1; page <= 5; page++) {
      try {
        const url = `${baseUrl}/offres?page=${page}`;
          ...
          if (elements.length > 0) {
            ...
            elements.each((index, element) => {
              try {
                ...
                const title = $el.find('h2, h3, .job-title, .title, a[href*="/offre/"]').first().text().trim();
                ...
                // Trouver le lien
                let jobUrl = $el.find('a[href*="/offre/"]').attr('href');
                if (!jobUrl) {
...
// Fonction alternative pour scraper via API
export async function scrapeKeejobAPI() {
  try {
    console.log('🔍 Tentative de scraping via API Keejob...');
    
    const response = await axiosInstance.get('https://www.keejob.com/api/offres', {
      params: {
        limit: 100,
        offset: 0
      }
